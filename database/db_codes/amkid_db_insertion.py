...
parser.add_argument("-a", "--all", dest="all", action="store_true",
        help='Tries to upload all in the sensors folder')
...
def read_file_type1(filename, date=True):
    ...
    data = data[np.invert(final_mask)]
    stamp = stamp[np.invert(final_mask)]
    #ipdb.set_trace()
...
class amkid_sensor_database():
    ...
    def fill_timestamps(self, timedata):
        ...
        self.time_dict['last_push'] = timedata['last_push']
        return 1
        ...
        for folder in folder_list:
            ...
            for keys, variable in self.Variable.items():
                ...
                for file, insert, dict_time_item, count in zip(variable['folders'],
                    ...
                    if(len(stamp)==0):
                        self.logger.info("item {} has zero data".format(
                                str(folder)+'/'+str(keys)+'/'+str(file),
                            ))
    ...
    def update_database(self, main_sensor_folder, file='Lakeshore/Temp1'):
        """
        ...
        folder_time = np.array([os.path.getmtime(os.path.join(x, file)) for x in folders])
...
if __name__ == '__main__':
    ...
    if(args.create):
        ...
        amkid_db = amkid_sensor_database(mysql_keys,
                sensor_folder=args.sensor_folder,
    ...
    if(args.update):
        ...
        amkid_db.update_database(args.sensor_folder)
    ...
    if(args.all):
        print("Trying to upload everything from the folder...")
        ##otherwise try to upload everything in the folder
        amkid_db = amkid_sensor_database(mysql_keys,
                sensor_folder=args.sensor_folder,
                timefile=args.timefile,
                logfile=args.logfile,
        ...
        folders = os.listdir(args.sensor_folder)
        folders.sort()
        folders = [os.path.join(args.sensor_folder, x) for x in folders]
